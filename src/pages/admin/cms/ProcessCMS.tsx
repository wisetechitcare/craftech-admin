...
export default function ProcessCMS() {
  ...
  const handleOpenModal = (
    step: any = { number: "", title: "", description: "" },
  ) => {
    setCurrentStep(step);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-ink-mute">
            Manage the 4-step technical blueprint.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-4">
        {steps.map((step) => (
          <div
            key={step._id}
            className="bg-paper border border-line rounded-xl p-5 flex justify-between items-center group"
          >
            <div className="flex items-center gap-6">
              <span className="text-3xl font-semibold text-ink group-hover:text-info transition-colors">
                {step.number}
              </span>
              <div>
          ...
          <div className="bg-paper border border-line rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <InputField
                ...
                onChange={(e) =>
                  setCurrentStep({ ...currentStep, number: e.target.value })
                }
              />
              <InputField
                label="Title"
