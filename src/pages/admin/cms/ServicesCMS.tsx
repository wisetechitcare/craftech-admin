    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-ink-mute">
            Manage technical services and expertises.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {items.map((item) => (
          <div
            key={item._id}
            className="bg-paper border border-line rounded-2xl p-6 relative group hover:border-ok/50 transition-all"
          >
            <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
              <button
                onClick={() => handleOpenModal(item)}
                className="p-2 bg-raise text-ink-soft rounded-xl hover:bg-line hover:text-ink"
              >
                <Edit2 className="w-4 h-4" />
              </button>
              <button
                onClick={() => handleDelete(item._id)}
                className="p-2 bg-danger/10 text-danger rounded-xl hover:bg-danger/10"
              >
            ...
            <div className="flex gap-6">
              <div className="w-16 h-16 bg-ok/10 rounded-2xl flex items-center justify-center text-ok text-3xl font-semibold shrink-0">
                {item.icon ? (
                  <i className={`fa-solid ${faIcon(item.icon)}`} />
                ) : (
                ...
                {item.features && item.features.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {item.features.map((f: string, i: number) => (
                      <span
                        key={i}
                        className="px-2 py-1 bg-raise/50 text-ink-soft text-[10px] font-bold rounded-md uppercase tracking-wider border border-line"
                      >
                        {f}
                      </span>
                    ))}
            ...
            <div className="mt-6 pt-4 border-t border-line flex justify-between items-center">
              <span
                className={`text-[10px] font-semibold px-2 py-1 rounded-full uppercase tracking-widest ${item.active ? "bg-ok/10 text-ok" : "bg-raise text-ink-soft-faint"}`}
              >
          ...
          <div className="bg-paper border border-line rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-line flex justify-between items-center bg-raise">
              ...
              <button
                ...
                className="text-ink-mute hover:text-ink p-2 hover:bg-raise rounded-full transition-colors"
              >
                <X />
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <TextArea
                label="Description"
                required
                labelClassName="text-[10px] font-semibold text-ink-mute uppercase tracking-widest"
                rows={3}
                ...
                <div className="flex items-end pb-1">
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <div className="relative">
                      ...
                      />
                      <div
                        className={`w-10 h-6 rounded-full transition-colors ${formData.active ? "bg-ok" : "bg-raise"}`}
                      />
                      <div
                        className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${formData.active ? "translate-x-4" : ""}`}
                      />
                    </div>
              ...
              <div className="pt-4 flex gap-4">
                <button
                  type="submit"
                  className="flex-1 py-4 bg-ok hover:bg-[#0c6543] text-white font-semibold rounded-2xl transition-all shadow-lg shadow-emerald-500/20"
                >
                ...
                <button
                  type="button"
                  onClick={() => setModalOpen(false)}
                  className="px-8 py-4 bg-paper hover:bg-raise text-ink-soft font-semibold rounded-2xl transition-all"
                >
                  Cancel
                </button>
              </div>
